...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, n_latent_var):
        ...
        self.Nfeet = action_dim//2
        self.foot_state = state_dim//self.Nfeet

        # actor
        self.action_sublayer = nn.Sequential(
                nn.Linear(self.foot_state, n_latent_var),
                nn.Tanh(),
    ...
    def action_layer(self,state):
        result = []
        for i in range(0,self.state_dim,self.foot_state):
            result.append(self.action_sublayer(state[i:i+self.foot_state]))
        return torch.cat(result,dim=0)
    
    def action_layer1(self,state):
        result = []
        for i in range(0,self.state_dim,self.foot_state):
            result.append(self.action_sublayer(state[:,i:i+self.foot_state]))
    ...
    def act(self, state, memory):
        ...
        action = []
        log_prob = []
        for i in range(0,2*self.Nfeet,2):
            dist = Categorical(action_probs[i:i+2])
            actionnow = dist.sample().reshape(1)
            action.append(actionnow)
            log_prob.append(dist.log_prob(actionnow))
