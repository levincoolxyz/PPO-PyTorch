...
class ActorCritic(nn.Module):
    def __init__(self, observation_dim, action_dim, action_std):
        ...
        """
        # cloned ants (assumed action_dim = 2*observation_dim) action range -1 to 1
        self.actor = nn.Sequential(
                torch.cat(tuple(nn.Linear(1, 64) for _ in range(observation_dim)), dim=1),
                nn.Tanh(),
                torch.cat(tuple(nn.Linear(64, 32) for _ in range(observation_dim)), dim=1),
                nn.Tanh(),
                torch.cat(tuple(nn.Linear(32, 2) for _ in range(observation_dim)), dim=1),
                nn.Tanh()
                )
        """
